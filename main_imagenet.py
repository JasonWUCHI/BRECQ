...
from models.my_model import MyTinyUNet
import sys
...
def get_data():
    ...
    dataloader = torch.utils.data.DataLoader(dataset=dataset, batch_size=32, shuffle=True, num_workers=4)
...
def calibrate_model(prepared_model, dataloader, num_timesteps):
    ...
    for batch, target in tqdm(dataloader):
        batch = batch
        timesteps = torch.randint(0, num_timesteps, (batch.shape[0],)).long()

        prepared_model(batch,timesteps)
        break
...
if __name__ == '__main__':
    ...
    cali_data = get_train_samples(dataloader, num_samples=args.num_samples)
